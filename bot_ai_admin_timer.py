...
ADMIN_ID = 7620745738  # замените на свой Telegram ID
...
async def start_handler(message: Message):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🛍 Мой шоп", url="https://t.me/RaindropShops")]
    ])
    if message.from_user.id == ADMIN_ID:
        kb.inline_keyboard.append([InlineKeyboardButton(text="🌐 Админ-панель", callback_data="admin")])
    
    await message.answer("Добро пожаловать! Этот бот создан для безопасных сделок. Владелец: @RaindropSpam_bot", reply_markup=kb)
